...
Based on Karlsson & Frank (2009) Nature Neuroscience
Reference: https://github.com/Eden-Kramer-Lab/ripple_detection
"""
...
def find_ripples_karlsson(lfp, fs, min_duration=0.015, zscore_thresh=3.0, smoothing_sigma=0.004, f_plot=0, threshold=None):
    '''
    Detect hippocampal ripples using Karlsson & Frank (2009) method
    ...
    min_duration : float
        Minimum time the z-score has to stay above threshold (seconds)
    zscore_thresh : float
        Number of standard deviations for ripple detection (default: 3.0)
    smoothing_sigma : float
        Smoothing window in seconds (default: 0.004)
    f_plot : bool
        Whether to plot detection results
    threshold : float or None
        Optional envelope threshold. If None, uses zscore_thresh
    
    Returns:
    --------
    dict : Detection results with keys:
        - filtered_lfp: Bandpass filtered signal (80-250 Hz)
        - StartIndex: Ripple start times (seconds)
        - PeakIndex: Ripple peak times (seconds)
        - EndIndex: Ripple end times (seconds)
        - thresh_envelope: Threshold value used
    '''
